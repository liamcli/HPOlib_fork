...
elif [ "$1" = "nvb_hyperband/nvb_hyperbandfix" ]; then
export searcher="nvb_hyperband/nvb_hyperband"
sudo PYTHONPATH=$PYTHONPATH HPOlib-run -o ../../../optimizers/$searcher -s $s --HPOLIB:experiment_directory_prefix open_ml_"$2"_data_seed_"$d_s"_fixB_ --HYPERBAND:fixb 1 --EXPERIMENT:openml_tid $2 --EXPERIMENT:data_split_seed $d_s
...
echo $1
echo $searcher
if [ "$searcher" = "smac/smac" ]; then
sudo rm -rf open_ml_"$2"_*_smac*_"$s"_*/*/state-run"$s"/*
fi
done
