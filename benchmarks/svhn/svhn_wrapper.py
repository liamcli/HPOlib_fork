...
data_dir="/home/lisha/school/Projects/hyperband_nnet/hyperband2/svhn"
...
def build_net(arm, split=0):
    ...
    n = caffe.NetSpec()
    if split==1:
        n.data, n.label = caffe.layers.Data(batch_size=arm['batch_size'], backend=caffe.params.Data.LMDB, source=data_dir+"/svhn_train",
                             transform_param=dict(mean_file=data_dir+"/mean.binaryproto"),ntop=2)
    elif split==2:
        n.data, n.label = caffe.layers.Data(batch_size=arm['batch_size'], backend=caffe.params.Data.LMDB, source=data_dir+"/svhn_val",
                             transform_param=dict(mean_file=data_dir+"/mean.binaryproto"),ntop=2)
    elif split==3:
        n.data, n.label = caffe.layers.Data(batch_size=arm['batch_size'], backend=caffe.params.Data.LMDB, source=data_dir+"/svhn_test",
                             transform_param=dict(mean_file=data_dir+"/mean.binaryproto"),ntop=2)
...
def build_solver(arm):
    ...
    s.snapshot = 10000
    s.snapshot_prefix = arm['dir']+"/svhn_data"
